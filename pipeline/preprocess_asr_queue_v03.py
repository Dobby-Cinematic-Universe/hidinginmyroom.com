...
def _validated_result_item(result_path: Path) -> dict[str, Any]:
    ...
    primary_streams = source_media_row["ffprobe_json"]["primary_streams"]
    source_has_audio = primary_streams["audio_index"] is not None
    normalized_audio_enabled = raw["processing_run"]["parameters_json"][
        "operations"
    ]["audio_flac"]
    ...
    audio_descriptor: dict[str, Any] | None = None
    if source_has_audio and normalized_audio_enabled:
        if len(audio_rows) != 1:
            raise QueueError(
                "audio-bearing preprocess result with normalized audio enabled must "
                "contain exactly one normalized audio artifact"
            )
        audio = audio_rows[0]
        artifact_id = _bounded_string(
            audio.get("artifact_id"), "audio artifact_id", 128
        )
        if not ARTIFACT_ID_RE.fullmatch(artifact_id):
            raise QueueError("audio artifact_id is malformed")
        digest = _sha256(audio.get("sha256"), "audio artifact SHA-256")
        byte_count = _integer(
            audio.get("byte_count"),
            "audio artifact byte_count",
            1,
            MAX_AUDIO_BYTES,
        )
        path = _absolute_path(audio.get("path"), "audio artifact path", existing=True)
        if (
            audio.get("storage_uri") != path.as_uri()
            or audio.get("visibility") != "private"
        ):
            raise QueueError("audio artifact URI or visibility is inconsistent")
        if (
            audio.get("media_kind") != "audio"
            or audio.get("mime_type") != "audio/flac"
        ):
            raise QueueError("audio artifact media/MIME kind is inconsistent")
        probe, duration_ms = _normalized_audio_probe(
            audio.get("normalized_probe"), digest, byte_count
        )
        ...
        if audio_rows:
            raise QueueError(
                "ASR-ineligible preprocess result unexpectedly contains normalized audio"
            )
        eligibility = {
            "eligible": False,
            "reason": (
                ...
                else "normalized_audio_operation_disabled"
            ),
            "source_audio_stream_present": source_has_audio,
            "normalized_audio_operation_enabled": normalized_audio_enabled,
        }
    ...
    if replay_path != resolved or replay_body != body:
        raise QueueError("preprocess result changed while its ASR eligibility was verified")
    routing_hint = None
    ...
    item = {
        ...
        "asr_eligibility": eligibility,
        "routing_hint": routing_hint,
        "raw_result": raw,
    }
    if audio_descriptor is not None:
        item["audio"] = audio_descriptor
    return item


def _validated_asr_eligibility(value: Any, label: str) -> dict[str, Any]:
    eligibility = _exact_object(
        ...
        label,
        {
            "eligible",
            "reason",
            "source_audio_stream_present",
            "normalized_audio_operation_enabled",
        },
    )
    if not isinstance(eligibility["eligible"], bool) or not isinstance(
        eligibility["source_audio_stream_present"], bool
    ) or not isinstance(eligibility["normalized_audio_operation_enabled"], bool):
        raise QueueError(f"{label} flags must be boolean")
    expected_reason = (
        None
        if eligibility["source_audio_stream_present"]
        and eligibility["normalized_audio_operation_enabled"]
        else (
            "source_has_no_audio"
            if not eligibility["source_audio_stream_present"]
            else "normalized_audio_operation_disabled"
        )
    )
    if eligibility["eligible"] is not (expected_reason is None) or eligibility[
        "reason"
    ] != expected_reason:
        raise QueueError(f"{label} disagrees with source streams and operation profile")
    return eligibility
...
def _collect_sealed_items(
    ...
    ineligible_receipts: list[dict[str, Any]] = []
    for ordinal in range(1, manifest["work_order_count"] + 1):
        ...
        eligibility = item.get("asr_eligibility")
        if eligibility is None and "audio" in item:
            # Compatibility for callers/tests that construct the historical
            # already-eligible internal item shape directly.
            eligibility = {
                "eligible": True,
                "reason": None,
                "source_audio_stream_present": True,
        ...
        eligibility = _validated_asr_eligibility(
            eligibility, f"receipt {ordinal} ASR eligibility"
        )
        receipt_audio = [
            row
        ...
        if eligibility["eligible"]:
            if len(receipt_audio) != 1 or "audio" not in item:
                raise QueueError(
                    f"ASR-eligible receipt {ordinal} has no unique normalized audio artifact"
                )
            audio = receipt_audio[0]
            for key in ("artifact_id", "path", "sha256", "byte_count"):
                if item["audio"][key] != audio[key]:
                    raise QueueError(
                        f"receipt {ordinal} audio {key} differs from the result"
                    )
            if item["audio"]["uri"] != audio["storage_uri"]:
                raise QueueError(f"receipt {ordinal} audio URI differs from the result")
        else:
            reason = eligibility["reason"]
            if receipt_audio or "audio" in item:
                raise QueueError(
                    f"ASR-ineligible receipt {ordinal} unexpectedly retains normalized audio"
                )
            ineligible_receipts.append(
                {
                    "ordinal": ordinal,
                    "entry_id": receipt["entry_id"],
                    "reason": reason,
                    "source_audio_stream_present": eligibility[
                        "source_audio_stream_present"
                    ],
                    "normalized_audio_operation_enabled": eligibility[
                        "normalized_audio_operation_enabled"
                    ],
                    "receipt": receipt_ref,
    ...
    origin = {
        ...
        **(
            ...
            if ineligible_receipts
            else {}
        ),
        **({"handling_control": control} if control is not None else {}),
    }
    eligible_ordinals = {
        item["evidence"]["receipt"]["ordinal"] for item in items
    }
    eligible_private_ordinals = set(control_by_ordinal) & eligible_ordinals
    if {
        item["evidence"]["receipt"]["ordinal"]
        for item in items
        if "handling" in item
    } != eligible_private_ordinals:
...
def _queue_handling_control(
    ...
    for item in items:
        ...
        if preprocess_ordinal in item_ordinals:
            raise QueueError("queue repeats a preprocess receipt ordinal")
        item_ordinals.add(preprocess_ordinal)
    ...
    expected_ordinals = set(by_ordinal) & item_ordinals
    if observed_ordinals != expected_ordinals:
        raise QueueError(
            "queue does not carry every eligible preprocess handling-control entry"
        )
    rows = [
        row for row in control["entries"] if row["ordinal"] in expected_ordinals
    ]
    if not rows:
        return None
    core = {
    ...
    return {**core, "identity_sha256": sha256_bytes(canonical_bytes(core))}
...
def build_queue(
    *,
    ...
    if sealed_requested:
        ...
        protected = [
            ...
            *[
                ...
                for path in (
                    skipped["preprocess_result"]["path"],
                    skipped["source_media"]["path"],
                )
            ],
        ]
    else:
    ...
    if not items and origin.get("ineligible_receipts"):
        reasons = sorted(
            {row["reason"] for row in origin["ineligible_receipts"]}
        )
        raise QueueError(
            "no ASR-eligible normalized audio artifacts remain after explicit skips; "
            f"reasons={reasons}"
        )
...
def _collect_catalog_items(
    ...
    ineligible = [
        item for item in items if not item["asr_eligibility"]["eligible"]
    ]
    if ineligible:
        reasons = sorted(
            {item["asr_eligibility"]["reason"] for item in ineligible}
        )
        raise QueueError(
            "catalog mode requires every explicit result to contain eligible normalized "
            f"audio; ineligible_count={len(ineligible)}, reasons={reasons}"
        )

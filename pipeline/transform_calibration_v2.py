#!/usr/bin/env python3
"""Private, catalog-free source/recording transform calibration evidence v0.2.
...
The v0.1 module remains byte-frozen at ``transform_calibration.py`` solely to replay
its hash-bound historical receipt. New production runs enter through this module.
"""
...
LEGACY_IMPLEMENTATION = REPOSITORY_ROOT / "pipeline" / "transform_calibration.py"
LEGACY_IMPLEMENTATION_SHA256 = (
    "7a6c1f1c7db417ddc7507731fee8230d471691b249627cf0c9fb34c3bcd1d969"
)
APPROVED_THRESHOLD_FLOORS = {
    "minimum_identity_ssim": 0.98,
    "minimum_identity_audio_correlation": 0.95,
    "minimum_visual_margin": 0.03,
    "minimum_audio_margin": 0.10,
}
...
def _load_legacy_implementation() -> Any:
    binding = regular_file(
        LEGACY_IMPLEMENTATION,
        "legacy transform-calibration implementation",
        1024 * 1024,
    )
    if binding["sha256"] != LEGACY_IMPLEMENTATION_SHA256:
        raise TransformCalibrationError(
            "legacy transform-calibration implementation SHA-256 differs"
        )
    spec = importlib.util.spec_from_file_location(
        "himr_transform_calibration_legacy_v0_1_0", LEGACY_IMPLEMENTATION
    )
    if spec is None or spec.loader is None:
        raise TransformCalibrationError(
            "cannot load legacy transform-calibration replay implementation"
        )
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module
...
def validate_acquisition_side(
    ...
    probe_streams = acquisition_probe["streams"]
    probe_types = [stream["codec_type"] for stream in probe_streams]
    if len(probe_streams) != 2 or probe_types.count("audio") != 1 or probe_types.count("video") != 1:
        raise TransformCalibrationError(
            f"{role} normalized acquisition probe must contain exactly one audio and one video stream"
        )
...
def ensure_ignored_output(path: Path) -> None:
    ...
    try:
        ...
        raise TransformCalibrationError(
            f"cannot prove receipt output path is symlink-free: {error}"
        ) from error
    if resolved_parent != path.parent:
        raise TransformCalibrationError(
            "receipt output parent must not contain symlink components"
        )
    if output_mode is not None and stat.S_ISLNK(output_mode):
        raise TransformCalibrationError("receipt output must not be a symlink")
    try:
        completed = subprocess.run(
...
def atomic_private_receipt(path: Path, body: bytes) -> None:
    ...
    try:
        ...
        directory = os.open(path.parent, open_flags)
    except OSError as error:
        raise TransformCalibrationError(
            f"cannot open private receipt output directory safely: {error}"
        ) from error
    ...
    if (
        ...
        os.close(directory)
        raise TransformCalibrationError(
            "private receipt output directory changed while opening"
        )
    try:
    ...
    if path.exists():
        try:
            ...
            if path.read_bytes() != body or existing["mode"] != 0o400:
                raise TransformCalibrationError(
                    "existing receipt differs or is not sealed mode 0400"
                )
            return
        finally:
            os.close(directory)
    ...
    try:
        ...
        os.chmod(temporary_name, 0o400, dir_fd=directory)
        os.replace(
            temporary_name,
            path.name,
...
def build_receipt(
    *,
    ...
    if old["yt_dlp"] != finalized["yt_dlp"]:
        raise TransformCalibrationError(
            "acquisitions do not share one observed yt-dlp launcher/version identity"
        )
...
def threshold(value: object, label: str, minimum: float) -> float:
    try:
        result = float(value)
    ...
    if not math.isfinite(result) or result < minimum or result > 1:
        raise TransformCalibrationError(
            f"{label} must be in the approved range [{minimum},1]"
        )
    return result
...
def validate_receipt(path: Path) -> dict[str, Any]:
    ...
    ensure_ignored_output(path)
    receipt, body = load_json(path, "receipt")
    implementation_version = receipt.get("implementation_version")
    if implementation_version == "0.1.0":
        legacy = _load_legacy_implementation()
        try:
            validated = legacy.validate_receipt(path)
        except Exception as error:
            raise TransformCalibrationError(
                f"legacy v0.1.0 receipt replay failed: {error}"
            ) from error
        for field, minimum in APPROVED_THRESHOLD_FLOORS.items():
            threshold(
                validated["recipe"]["thresholds"][field],
                field.replace("_", " "),
                minimum,
            )
        return validated
    if implementation_version != IMPLEMENTATION_VERSION:
        raise TransformCalibrationError(
            "receipt implementation version is unsupported"
        )

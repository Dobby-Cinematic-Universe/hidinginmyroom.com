...
TEST_ROOT = PIPELINE_ROOT / ".test-work" / f"preprocess-asr-runner-v03-{os.getpid()}"
sys.path.insert(0, str(PIPELINE_ROOT))

import asr_whispercpp  # noqa: E402
import preprocess_asr_queue_v03 as queue  # noqa: E402
import preprocess_asr_queue_runner_v03 as runner  # noqa: E402
...
class PreprocessASRQueueRunnerTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.origin = self._origin(len(self.items) + 1)
        skipped_source = TEST_ROOT / "fixture-source-no-audio.mp4"
        skipped_result = TEST_ROOT / "fixture-result-no-audio.json"
        skipped_source.write_bytes(b"video-only source fixture\n")
        skipped_result.write_bytes(b"{}\n")
        self.origin["ineligible_receipts"] = [
            {
                ...
                "receipt": {
                    "path": str(TEST_ROOT / "receipt-no-audio.json"),
                    "uri": (TEST_ROOT / "receipt-no-audio.json").as_uri(),
                    "physical_sha256": "1" * 64,
                    "receipt_id": "ppreceipt_" + "2" * 32,
                    "receipt_sha256": "3" * 64,
                    "ordinal": len(self.items) + 1,
                },
                "preprocess_result": {
                    "path": str(skipped_result),
                    "uri": skipped_result.as_uri(),
                    "sha256": digest(skipped_result),
                    "byte_count": skipped_result.stat().st_size,
                    "job_id": "preprocess-no-audio-fixture",
                    "processing_run_id": "run_preprocess_" + "4" * 32,
                    "recipe_sha256": "5" * 64,
                },
                "source_media": {
                    "media_id": "media_sha256_" + digest(skipped_source),
                    "sha256": digest(skipped_source),
                    "byte_count": skipped_source.stat().st_size,
                    "path": str(skipped_source),
                    "storage_uri": skipped_source.as_uri(),
                    "duration_ms": 1_000,
    ...
    def _validate_summary_schema(self, value: dict[str, object]) -> None:
        schema = json.loads(
            (PIPELINE_ROOT / "schemas/preprocess-asr-queue-run-v03.schema.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_skip_bearing_manifest_replays_and_runner_admits_eligible_subset(self) -> None:
        manifest, orders = queue.validate_queue(self.manifest_path)
        self.assertEqual(5, manifest["origin"]["receipt_count"])
        self.assertEqual(
            "source_has_no_audio",
            manifest["origin"]["ineligible_receipts"][0]["reason"],
        )
        self.assertEqual(4, len(orders))
        summary = runner.validate_dispatcher(self.manifest_path)
        self.assertEqual("validated", summary["status"])
        self.assertEqual(4, summary["job_count"])
        self._validate_summary_schema(summary)

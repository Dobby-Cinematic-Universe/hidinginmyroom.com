...
TEST_ROOT = PIPELINE_ROOT / ".test-work" / f"asr-result-store-seal-v05-{os.getpid()}"
sys.path.insert(0, str(PIPELINE_ROOT))

import asr_whispercpp_result_store_seal_v05 as seal  # noqa: E402
...
class ResultStoreSealTests(unittest.TestCase):
    ...
    def test_v05_replay_treats_persisted_object_numbers_and_times_as_diagnostic(self) -> None:
        fixture = SyntheticStore()
        ...
        for result in plan["results"]:
            ...
            for item in result["files"]:
                ...
                item["mtime_ns"] = 0
        drifted_plan = repin_plan(fixture, plan)
        self.assertEqual(
            "valid_prepared_plan",
            seal.validate_plan(
                drifted_plan,
        ...
        receipt["identity_sha256"] = seal._document_identity(semantic, set())
        receipt_path.chmod(0o600)
        write_json(receipt_path, receipt, 0o400)
        self.assertEqual(
            "valid_applied_receipt",
            seal.validate_receipt(
                receipt_path,
    ...
    def test_authoritative_queue_wrapper_calls_materializer_validator(self) -> None:
        import preprocess_asr_queue_v03

        manifest_path = TEST_ROOT / "sealed-queue" / "manifest.json"
        expected = ({"queue_id": "test"}, [{"job_id": "job"}])
        with mock.patch.object(
            preprocess_asr_queue_v03, "validate_queue", return_value=expected
        ) as validate:
    ...
    def test_private_current_plan_validates_when_available(self) -> None:
        ...
        expected_implementation = seal._file_identity(Path(seal.__file__).resolve())
        if plan.get("implementation") != expected_implementation:
            self.skipTest("private seal plan belongs to a different implementation")
        receipt_path = Path(plan["receipt_path"])
        result = (

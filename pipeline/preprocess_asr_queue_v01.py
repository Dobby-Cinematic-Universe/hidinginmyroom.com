...
It writes only private JSON queue manifests and ordinary ASR work orders.
"""
...
def validate_queue(manifest_path: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    supplied = _exact_object(parse_json(body, "queue manifest"), "queue manifest", MANIFEST_KEYS)
    if (
    ...
    for ordinal, order_body in enumerate(bodies, 1):
        ...
        try:
            ...
            raise QueueError(f"ASR work order {ordinal} is invalid: {error}") from error
        entry = rebuilt["work_orders"][ordinal - 1]
        if sha256_bytes(canonical_bytes(order)) != entry["canonical_sha256"]:
            raise QueueError(f"ASR work order {ordinal} canonical hash differs from its pin")

"""Separate finite metadata-guided CPU/CUDA screening with mandatory baseline probes.

Metadata affects queue order and additive probe locations, never speaker labels.
...
from pipeline import speaker_screen_guided_core as guided_core
...
MARKER = {"kind": "himr_private_guided_speaker_screen_workspace", "schema_version": 1}
IMPLEMENTATION_NAMES = resident.IMPLEMENTATION_NAMES + (
    "speaker_screen_guided.py", "speaker_screen_guided_core.py", "speaker_screen_guidance.py")
...
def build_manifest(request, *, request_binding=None):
    ...
    if (request["kind"] != "himr_guided_speaker_screen_request"
    ...
    for index, original_index in enumerate(indices):
        ...
        value = {"kind": "himr_guided_speaker_screen_plan", "schema_version": 1,
        ...
        plans.append({**value, "plan_id": "guidedscreen_" + screen.digest(value)[:32]})
    value = {"kind": "himr_guided_speaker_screen_manifest", "schema_version": 1,
             ...
             "semantics": {"visibility": "private", "discovery": False, "controller_mutation": False,
                 ...
                 "metadata_is_not_speaker_evidence": True, "baseline_sampling_preserved": True,
                 "positive_early_stop_requires_complete_baseline": True}}
    manifest = {**value, "batch_id": "guidedbatch_" + screen.digest(value)[:32]}
...
def validate_manifest(value):
    ...
    request = {"kind": "himr_guided_speaker_screen_request", "schema_version": 1,
...
def _locked(root):
    with paths.retained_directory(root) as directory:
        if screen.read_json(root / "workspace.json") != MARKER:
            raise ScreenError("guided workspace marker differs")
        fd = os.open("guided.lock", os.O_RDWR | os.O_CREAT | os.O_NOFOLLOW | os.O_CLOEXEC, 0o600, dir_fd=directory)
        try:
            _check_lock(fd)
            try:
...
def _observations_for(plan, job, binding, *, cache=None):
    ...
    for index, windows in enumerate(plan["batches"]):
        ...
        if (value["kind"] != "himr_guided_speaker_screen_checkpoint"
...
def _result_document(plan, binding, observations, proofs, runtime, summary):
    ...
    return {"kind": "himr_guided_speaker_screen_result", "schema_version": 1,
...
def _summary(manifest, rows, *, invocation_state=None, errors=None, metrics=None):
    ...
    return {"kind": "himr_guided_speaker_screen_status", "schema_version": 1,
...
def status_batch(path, expected):
    ...
    with paths.retained_directory(root) as directory:
        ...
        try:
            try:
                descriptor = os.open("guided.lock", os.O_RDONLY | os.O_NOFOLLOW | os.O_NONBLOCK, dir_fd=directory)
...
def _run_number(root, manifest):
    ...
    for number, name in enumerate(starts, 1):
        ...
        if start != {"kind": "himr_guided_speaker_screen_run", "schema_version": 1,
    ...
    with screen.opened(order["recording"]["path"]) as source, screen.opened(order["ffmpeg"]["path"], executable=True) as ffmpeg:
        ...
        with paths.retained_directory(job):
            ...
            for ordinal, (index, windows) in enumerate(pending):
                ...
                checkpoint = {"kind": "himr_guided_speaker_screen_checkpoint", "schema_version": 1,
...
def run_batch(path, expected):
    ...
    with old_batch._cancellable(), _locked(root) as lock_fd:
        ...
        start = {"kind": "himr_guided_speaker_screen_run", "schema_version": 1,
                 "batch_id": manifest["batch_id"], "run": number}
        ...
        _write(root / f"run-{number:06d}.finish.json", {
            "kind": "himr_guided_speaker_screen_run_result", "schema_version": 1,

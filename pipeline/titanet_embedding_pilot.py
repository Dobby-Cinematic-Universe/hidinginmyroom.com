...
def select(segments):
    """Up to three disjoint 5s excerpts per label, inside 250ms turn guards."""
    ...
    for label in labels:
        ...
        for i, s in enumerate(segments):
            if s['speaker'] != label or s['end_ms'] - s['start_ms'] < 5500:
                continue
            start = (s['start_ms'] + s['end_ms'] - 5000) // 2
            end = start + 5000
        ...
        for label in dict.fromkeys(s['speaker'] for s in doc['segments']):
            if not any(c['provider_label'] == label for c in selected):
                skipped.append({'recording_id': doc['recording_id'], 'provider_label': label,
                                'reason': 'no_nonoverlapping_5s_excerpt_with_turn_guards'})
        for c in selected:
            if len(rows) >= 48:

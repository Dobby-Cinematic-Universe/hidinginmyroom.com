...
PREPROCESS_QUEUE_SOURCE = REPOSITORY_ROOT / "pipeline" / "preprocess_asr_queue_v03.py"
PREPROCESS_QUEUE_SCHEMA_SOURCE = (
    REPOSITORY_ROOT
    / "pipeline"
    / "schemas"
    / "preprocess-asr-queue-manifest-v03.schema.json"
)
...
def authoritative_queue_validate(
    ...
    from preprocess_asr_queue_v03 import validate_queue  # noqa: PLC0415
...
def _open_planned_results(
    ...
    *,
    ...
    for expected_ordinal, expected in enumerate(plan["results"], 1):
        ...
        # Persisted object numbers/timestamps are diagnostic only. The retained
        # descriptor and path are still one object for this complete operation.
...
def _after_file_record(
    ...
    *,
    ...
    value = retained.verify(
        ...
        expected_mtime_ns=retained.opened.st_mtime_ns,
    ...
    return {
        ...
        "mtime_ns": value.st_mtime_ns,
        "mtime_unchanged": value.st_mtime_ns == retained.opened.st_mtime_ns,
    ...
    *,
    ...
    for retained_result, expected in zip(retained_results, plan["results"], strict=True):
        ...
        directory = retained_result.directory.verify(
            ...
            expected_mtime_ns=retained_result.directory.opened.st_mtime_ns,
        ...
        result_records.append(
            {
                "directory": {
                    ...
                    == retained_result.directory.opened.st_mtime_ns,
            ...
            for result, expected, ctimes in zip(
                ...
                for expected_file in expected["files"]:
                    result.files[expected_file["name"]].verify(
                        modes={RESULT_FILE_MODE_AFTER},
                        expected_sha256=expected_file["sha256"],
                        expected_mtime_ns=result.files[
                            expected_file["name"]
                        ].opened.st_mtime_ns,
                ...
                result.directory.verify(
                    modes={RESULT_DIRECTORY_MODE_AFTER},
                    entries=set(RESULT_FILENAMES),
                    expected_mtime_ns=result.directory.opened.st_mtime_ns,
...
def _portable_receipt_result(value: dict[str, Any]) -> dict[str, Any]:
    """Return restart-stable receipt authority, retaining object IDs as diagnostics.

    ``device``, ``inode``, ``mtime``, and post-chmod ``ctime`` describe the
    filesystem object observed while the receipt was created. They are useful
    forensic evidence but are not stable across reboot, remount, or
    content-preserving relocation. Live descriptor/path identity and timestamps are
    still checked by every ``Retained*`` verification.
    """

    diagnostic = {"ctime_ns_after", "device", "inode", "mtime_ns"}
    return {
        **{
            key: item
            for key, item in value.items()
            if key not in {"directory", "files"}
        },
        "directory": {
            key: item
            for key, item in value["directory"].items()
    ...
    *,
    ...
    with ExitStack() as stack:
        ...
        for actual, expected in zip(receipt["results"], observed["results"], strict=True):
            if _portable_receipt_result(actual) != _portable_receipt_result(expected):

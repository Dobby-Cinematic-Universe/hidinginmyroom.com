...
test('retrieval uses strict similarity cache and rejects partial provider failures',async()=>{
  let options;
  ...
  assert.equal(options.cache.cache_threshold,'super_strict_match');
  assert.equal(options.retrieval.return_on_failure,false);
});
test('empty cached retrieval retries once without the similarity cache',async()=>{
  const calls=[];
  const env={AI_SEARCH:{get:()=>({search:async request=>{
    calls.push(request.ai_search_options.cache);
    return {chunks:calls.length===1?[]:[{text:'Evidence',item:{key:'source'}}]};
  }})}};
  assert.equal((await retrieve(env,'test question')).length,1);
  assert.deepEqual(calls,[{enabled:true,cache_threshold:'super_strict_match'},{enabled:false,cache_threshold:'super_strict_match'}]);
});
